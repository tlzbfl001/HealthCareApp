...
class DataManager(private var context: Context?) {
   ...
   fun getUser(type: String, email: String) : User {
      ...
      while(cursor.moveToNext()) {
         ...
         data.birthday = cursor.getString(6)
         data.image = cursor.getString(7)
         data.height = cursor.getDouble(8).toString()
         data.weight = cursor.getDouble(9).toString()
         data.weightGoal = cursor.getDouble(10).toString()
         data.kcalGoal = cursor.getInt(11).toString()
         data.waterGoal = cursor.getInt(12).toString()
         data.waterUnit = cursor.getInt(13).toString()
         data.regDate = cursor.getString(14)
      }
      cursor.close()
      return data
   }
   ...
   fun updateBody(data: Body){
      ...
      val sql = "update $TABLE_BODY set height=${data.height}, weight=${data.weight}, age=${data.age}, gender='${data.gender}', " +
         "exerciseLevel=${data.exerciseLevel}, fat=${data.fat}, muscle=${data.muscle}, bmi=${data.bmi}, bmr=${data.bmr} where userId = ${MyApp.prefs.getId()} and id=${data.id}"
      db.execSQL(sql)
      db.close()
   }
