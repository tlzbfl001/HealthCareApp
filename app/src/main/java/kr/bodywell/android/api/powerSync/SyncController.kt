	...
	suspend fun upsert(context: Context, entry: CrudEntry) {
		...
		when(entry.table) {
			FOODS -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createFood: ${response.body()}") else Log.e(TAG, "createFood: $response")
			}
			DIETS -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createDiets: ${response.body()}") else Log.e(TAG, "createDiets: $response")
			}
			WATER -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createWater: ${response.body()}") else Log.e(TAG, "createWater: $response")
			}
			ACTIVITIES -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createActivity: ${response.body()}") else Log.e(TAG, "createActivity: $response")
			}
			WORKOUTS -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createWorkout: ${response.body()}") else Log.e(TAG, "createWorkout: $response")
			}
			BODY_MEASUREMENTS -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createBody: ${response.body()}") else Log.e(TAG, "createBody: $response")
			}
			SLEEP -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createSleep: ${response.body()}") else Log.e(TAG, "createSleep: $response")
			}
			MEDICINES -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createMedicine: ${response.body()}") else Log.e(TAG, "createMedicine: $response")
			}
			MEDICINE_TIMES -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createMedicineTime: ${response.body()}") else Log.e(TAG, "createMedicineTime: $response")
			}
			MEDICINE_INTAKES -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createMedicineIntake: ${response.body()}") else Log.e(TAG, "createMedicineIntake: $response")
			}
			GOALS -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createGoal: ${response.body()}") else Log.e(TAG, "createGoal: $response")
			}
			NOTES -> {
				...
				if(response.isSuccessful) Log.i(TAG, "createNote: ${response.body()}") else Log.e(TAG, "createNote: $response")
			}
			FILES -> {
	...
	suspend fun update(entry: CrudEntry) {
		...
		when(entry.table) {
			PROFILES -> {
				...
				if(response.isSuccessful) Log.i(TAG, "updateProfile: ${response.body()}") else Log.e(TAG, "updateProfile: $response")
			}
			FOODS -> {
			...
			DIETS -> {
				...
				if(getDiets.isSuccessful) {
					...
					if(response.isSuccessful) Log.i(TAG, "updateDiets: ${response.body()}") else Log.e(TAG, "updateDiets: $response")
				}
			}
			WATER -> {
				...
				if(getWater.isSuccessful) {
					...
					if(response.isSuccessful) Log.i(TAG, "updateWater: ${response.body()}") else Log.e(TAG, "updateWater: $response")
				}
			}
			WORKOUTS -> {
				...
				if(getWorkout.isSuccessful) {
					...
					if(response.isSuccessful) Log.i(TAG, "updateWorkout: ${response.body()}") else Log.e(TAG, "updateWorkout: $response")
				}
			}
			BODY_MEASUREMENTS -> {
				...
				if(getBody.isSuccessful) {
					...
					if(response.isSuccessful) Log.i(TAG, "updateBody: ${response.body()}") else Log.e(TAG, "updateBody: $response")
				}
			}
			SLEEP -> {
			...
			MEDICINES -> {
				...
				if(getMedicine.isSuccessful) {
					...
					if(response.isSuccessful) Log.i(TAG, "updateMedicine: ${response.body()}") else Log.e(TAG, "updateMedicine: $response")
				}
			}
			NOTES -> {
				...
				if(getNote.isSuccessful) {
					...
					if(response.isSuccessful) Log.i(TAG, "updateNote: ${response.body()}") else Log.e(TAG, "updateNote: $response")
				}
			}
			GOALS -> {
				...
				if(getGoal.isSuccessful) {
					...
					if(response.isSuccessful) Log.i(TAG, "updateGoal: ${response.body()}") else Log.e(TAG, "updateGoal: $response")
				}
			}
		}
	}
	...
	suspend fun delete(table: String, id: String) {
		when(table) {
			FOODS -> {
				...
				if(response.isSuccessful) Log.i(TAG, "deleteFood: ${response.raw().code}") else Log.e(TAG, "deleteFood: $response")
			}
			DIETS -> {
				val response = RetrofitAPI.api.deleteDiet("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteDiet: ${response.raw().code}") else Log.e(TAG, "deleteDiet: $response")
			}
			WATER -> {
				val response = RetrofitAPI.api.deleteWater("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteWater: ${response.raw().code}") else Log.e(TAG, "deleteWater: $response")
			}
			ACTIVITIES -> {
				val response = RetrofitAPI.api.deleteActivity("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteActivity: ${response.raw().code}") else Log.e(TAG, "deleteActivity: $response")
			}
			WORKOUTS -> {
				val response = RetrofitAPI.api.deleteWorkout("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteWorkout: ${response.raw().code}") else Log.e(TAG, "deleteWorkout: $response")
			}
			BODY_MEASUREMENTS -> {
				val response = RetrofitAPI.api.deleteBody("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteBody: ${response.raw().code}") else Log.e(TAG, "deleteBody: $response")
			}
			MEDICINES -> {
				val response = RetrofitAPI.api.deleteMedicine("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteMedicine: ${response.raw().code}") else Log.e(TAG, "deleteMedicine: $response")
			}
			MEDICINE_TIMES -> {
				val response = RetrofitAPI.api.deleteMedicineTime("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteMedicineTime: ${response.raw().code}") else Log.e(TAG, "deleteMedicineTime: $response")
			}
			MEDICINE_INTAKES -> {
				val response = RetrofitAPI.api.deleteMedicineIntake("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteMedicineIntake: ${response.raw().code}") else Log.e(TAG, "deleteMedicineIntake: $response")
			}
			FILES -> {
				val response = RetrofitAPI.api.deleteFile("Bearer ${getToken.access}", id)
				if(response.isSuccessful) Log.i(TAG, "deleteFile: ${response.raw().code}") else Log.e(TAG, "deleteFile: $response")
			}
		}
	}
}