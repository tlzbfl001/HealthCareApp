...
class DBHelper(context: Context?) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
   companion object {
      ...
      const val TABLE_SYNC_DATA = "syncData"
   ...
   override fun onCreate(db: SQLiteDatabase) {
      val user = "create table $TABLE_USER(id integer primary key autoincrement, type text, email text, idToken text, name text, gender text, birthday text, image text, " +
      ...
      val water = "create table $TABLE_WATER(id integer primary key autoincrement, $USER_ID integer, uid text, count integer, volume integer, " +
         "$CREATED_AT text, $UPDATED_AT text, $IS_UPDATED integer);"
      db.execSQL(water)
      ...
      val drugCheck = "create table $TABLE_DRUG_CHECK(id integer primary key autoincrement, $USER_ID integer, uid text, drugId integer, drugTimeId integer, $CREATED_AT text);"
      db.execSQL(drugCheck)

      val note = "create table $TABLE_NOTE(id integer primary key autoincrement, $USER_ID integer, title text, content integer, status integer, $CREATED_AT text);"
      db.execSQL(note)
      ...
      val image = "create table $TABLE_IMAGE(id integer primary key autoincrement, $USER_ID integer, type text, dataId integer, imageUri text, $CREATED_AT text);"
      db.execSQL(image)

      val unused = "create TABLE $TABLE_UNUSED(id integer primary key autoincrement, $USER_ID integer, type text, value text, drugUid text, drugTimeUid text, $CREATED_AT text);"
      db.execSQL(unused)

      val syncData = "create TABLE $TABLE_SYNC_DATA(id integer primary key autoincrement, $USER_ID integer, syncedAt text);"
      db.execSQL(syncData)
   }
