...
import kr.bodywell.health.api.powerSync.SyncController.delete
import kr.bodywell.health.api.powerSync.SyncController.upsert
import kr.bodywell.health.api.powerSync.SyncController.update
...
import kr.bodywell.health.model.Constant.ACTIVITY_USAGES
...
import kr.bodywell.health.model.Constant.FOOD_USAGES
...
//					Log.d(TAG, "${entry.op}\n${entry.id}\n${entry.opData}")
	...
	fun watchAllMedicine(): Flow<List<Medicine>> {
		return database.watch("SELECT id, category, name, amount, unit, strftime('%Y-%m-%d', starts), strftime('%Y-%m-%d', ends) " +
			"FROM $MEDICINES WHERE user_id = '${getUser.uid}'", mapper = { cursor ->
			Medicine(
	...
	suspend fun getActivities(): List<ActivityData> {
		return database.getAll(sql = "SELECT * FROM(SELECT id, name, register_type, created_at FROM $ACTIVITIES " +
			"WHERE user_id = '${getUser.uid}') GROUP BY name ORDER BY created_at", mapper = { cursor ->
			ActivityData(
				id = if(cursor.getString(0) == null) "" else cursor.getString(0)!!,
				name = if(cursor.getString(1) == null) "" else cursor.getString(1)!!,
				registerType = if(cursor.getString(2) == null) "" else cursor.getString(2)!!
	...
	suspend fun getFile(data: String): FileItem {
		return database.getOptional(sql = "SELECT id, name, data FROM $FILES WHERE user_id = '${getUser.uid}' AND profile_id = '$data' ORDER BY created_at DESC LIMIT 1", mapper = { cursor ->
			FileItem(
				id = if(cursor.getString(0) == null) "" else cursor.getString(0)!!,
				name = if(cursor.getString(1) == null) "" else cursor.getString(1)!!,
