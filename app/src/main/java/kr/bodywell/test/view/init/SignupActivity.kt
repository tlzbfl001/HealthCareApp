...
import kr.bodywell.test.databinding.ActivitySignupBinding
...
import kr.bodywell.test.util.CustomUtil.Companion.isoFormat2
...
//                     registerUser2()
   ...
   private fun registerUser() {
      ...
      // 사용자 정보 저장
      if(getUser.created == "") {
         dataManager.insertUser(User(type = user.type, email = user.email, idToken = user.idToken, userUid = userUid, created = LocalDate.now().toString(), updated = updated, isUpdated = 1))
      }else {
         dataManager.updateUser(User(type = user.type, email = user.email, idToken = user.idToken, userUid = userUid, created = LocalDate.now().toString(), updated = updated, isUpdated = 1))
      }
...
//         val createDevice = RetrofitAPI.api.createDevice("Bearer $access", data)
         val getAllDevice = RetrofitAPI.api.getAllDevice("Bearer $access")
         ...
         Log.d(TAG, "createDevice: ${getAllDevice.isSuccessful}/${getAllDevice.body()}")
         ...
         if(getAllDevice.isSuccessful && getProfile.isSuccessful && getAllFood.isSuccessful && getAllActivity.isSuccessful && getGoal.isSuccessful) {
            MyApp.prefs.setPrefs("userId", getUser.id) // 사용자 Id 저장
            dataManager.updateUserStr("deviceUid", getAllDevice.body()!![0].uid) // deviceUid 저장
            dataManager.updateUserStr("profileUid", getProfile.body()!!.uid) // profileUid 저장
            dataManager.insertGoal(Goal(uid = getGoal.body()!!.uid, created = LocalDate.now().toString()))
